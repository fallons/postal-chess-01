...
//---------------------------------------------------
  ...
  $.ajax({
  url: 'https://still-shelf-78251.herokuapp.com/writelog',
  type: 'POST',
  data: {
  log_tod:     g_tod,
  log_uname:   g_uname,
...
return new Promise((resolve, reject) => {
$.ajax({
url: 'https://young-hamlet-57834.herokuapp.com/readallusers',
type: 'POST',
data: {
key: 'value',
},
...
//==================================================================================================
// stackpop form handler
//==================================================================================================
function stackpopFormHandler(formObject)  {
call_stackpop();
}
//==================================================================================================
// stackpush form handler
//==================================================================================================
function stackpushFormHandler(formObject)  {
var stackpush = formObject.elements.stackpush.value;
call_stackpush(stackpush);
}

//==================================================================================================
// getstackptr form handler
//==================================================================================================
function getstackptrFormHandler(formObject)  {
call_getstackptr();
}

//==================================================================================================
// test_app_lock form handler
//==================================================================================================
function test_app_lockFormHandler(formObject)  {
call_test_app_lock();
}
//==================================================================================================
// test_app_lock form handler
//==================================================================================================
function get_app_lockFormHandler(formObject)  {
var get_app_lock = formObject.elements.get_app_lock.value;
call_get_app_lock();
}
//==================================================================================================
// test_app_lock form handler
//==================================================================================================
function free_app_lockFormHandler(formObject)  {
var  free_app_lock = formObject.elements.free_app_lock.value;
call_free_app_lock();
}
...
// test_app_lock form handler
//==================================================================================================
function lockable_actionFormHandler(formObject)  {
var  lockable_action = formObject.elements.lockable_action.value;
call_lockable_action();
}

//==================================================================================================
// test_app_lock form handler
//==================================================================================================
function  force_freeFormHandler(formObject)  {
var  lockable_action = formObject.elements.force_free.value;
call_force_free();
}

//==================================================================================================
// diag target square list
// for test use from diags tab only
//==================================================================================================
function formSubmitHandler4(formObject)  {
dlog('==> formSubmitHandler4(formObject');
//var idcol = formObject.elements.idcol.value;
var idxy = formObject.elements.idxy.value;
//wlog("idcol " + idcol);
dlog("idxy " + idxy);
//var thpname = "t"+idcol+"0";
isSqThreatened('zz0',idxy);  //fc
var sf1 = 1;
dlog('<== formSubmitHandler4');
}
...
// 
//==================================================================================================
function formLoginHandler(formObject)  {
  var new_name = formObject.elements.new_name.value;
  var sf1 = 1;
  }
...
//==============================================================================================================        
function diagemail() {
//let mysubject = document.getElementById("chesslogsubject").innerHTML;
...
$.post('https://still-shelf-78251.herokuapp.com/testemail',
{
toaddr: toaddr,
subject: mysubject,
mytext: mytext,
...
console.log('from still-shelf make socket.io connection to https://still-shelf-78251.herokuapp.com as "socket"');
var socket = io.connect('https://still-shelf-78251.herokuapp.com');
...
$(function(){
    //make connection
      console.log('from still-shelf - make socket.io connection to https://peaceful-earth-87247.herokuapp.com as "socket1"');
      var socket1 = io.connect('https://peaceful-earth-87247.herokuapp.com/');
      ...
      { 
        ...
        return -1; 
         ...
         //wmsg("Click on a pawn icon above to choose your playing colour");
...
case "check":
  ...
  break;
case "ding" :
  var beepsound = new Audio('ding.wav'); 
  beepsound.play(); 
  } 
}
